...
anomalies_file_path_pattern = "resources/data/4-anomalies-in-csv-shuffled/e{fault_code}-{pair_id}.csv"

premise_exp_folder_path_pattern_faulty = "resources/data/15-premise-data/anomalies-{model}/{data_set_type}/{id}-10.40.7.172-{fault_type}@{fault_pattern}_{faulty_pair_id}_{fault_pattern}"
premise_exp_folder_path_pattern_normal = "resources/data/15-premise-data/anomalies-{model}/{data_set_type}/{id}-10.40.7.172-failurefree_x_x"
...
fault_code_list = [[[1, 2, 3, 4, 6], [7, 8, 9, 10]], [[7, 8, 9, 10], [1, 2, 3, 4, 6]]]
# TODO: to train model on [1, 2, 3, 4, 5] and test on [7, 8, 9, 10]
# TODO: to recall why n8a_1__eX_04.txt anomalies used in premise instead of n8a_1__eX_1.txt)
fault_code_list = [[[1, 2, 3, 4, 5], [7, 8, 9, 10]], [[7, 8, 9, 10], [1, 2, 3, 4, 5]]]

fault_code_list = [[[], []], [[], [5]]]

faults = ["MemL-Exp", "MemL-Lin", "MemL-Rnd", "PacL-Lin", "PacL-Exp", "PacL-Exp", "PacL-Rnd", "CpuH-Exp", "CpuH-Lin", "CpuH-Rnd"]
...
for model in range(len(fault_code_list)):
    create_folder("resources/data/15-premise-data/anomalies-{model}".format(model=model))

    exp_unique_code = -1

    # Loop by data-sets [0 - training, 1 - test]
    for data_set_type in range(len(fault_code_list[model])):
        create_folder("resources/data/15-premise-data/anomalies-{model}/{data_set_type}".format(model=model, data_set_type=data_set_types[data_set_type]))
        ...
        for fault_code in fault_code_list[model][data_set_type]:
            ...
            for pair_id in pair_id_list:
                ...
                anomalies_file_path = os.path.join(script_dir, anomalies_file_path_pattern.format(fault_code=fault_code, pair_id=pair_id))
                ...
                if data_set_type == 0:
                    if faulty_codes_counter == 1:
                        ...
                        premise_exp_folder_path_normal = os.path.join(script_dir, premise_exp_folder_path_pattern_normal.format(model=model, data_set_type=data_set_types[data_set_type], id=exp_id))
                        create_folder(premise_exp_folder_path_normal)

                exp_unique_code += 1
                exp_id = gen_exp_unique_code(exp_unique_code)
                premise_exp_folder_path_faulty = os.path.join(script_dir, premise_exp_folder_path_pattern_faulty.format(model=model, data_set_type=data_set_types[data_set_type], id=exp_id, fault_type=fault_type, faulty_pair_id=pair_id, fault_pattern=fault_pattern))
